...
public class StackTools_v1_17_R1 implements IStackTools {
    ...
    public byte[] itemToBinary(ItemStack itemStack) throws IOException {
        ...
        compoundTag.setInt(NYAACORE_ITEMSTACK_DATAVERSION_KEY, currentDataVersion);
    ...
    public ItemStack itemFromBinary(byte[] nbt, int offset, int len) throws IOException {
        ...
        int dataVersion = reconstructedCompoundTag.getInt(NYAACORE_ITEMSTACK_DATAVERSION_KEY); // nbt.getInt
        if (dataVersion > 0) {
            reconstructedCompoundTag.remove(NYAACORE_ITEMSTACK_DATAVERSION_KEY); // nbt.remove
        }
