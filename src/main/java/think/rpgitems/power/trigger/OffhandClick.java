...
class OffhandClick extends Trigger<PlayerInteractEvent, PowerOffhandClick, Void, Void> {
    ...
    @Override
    public PowerResult<Void> run(RPGItem item, PowerOffhandClick power, Player player, ItemStack i, PlayerInteractEvent event) {
        return power.offhandClick(player, item, i, event);
    }
}
